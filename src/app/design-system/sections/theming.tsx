 ...
 * the F2 review requires. A dual-palette tenant (the operator brand) shows
 * BOTH mode decisions — each palette passes the same gate independently.
 */
...
import type { OperatorMode } from "@/lib/theme/operator-theme";
...
/** One resolution's verdict + corrections + contrast report. */
function EngineDecision({ resolution }: { resolution: TenantThemeResolution }) {
  return (
...
export function ThemingSection({
  ...
  /** Dual-palette tenant: per-mode resolutions (operator brand). */
  modeResolutions?: Record<OperatorMode, TenantThemeResolution> | null;
}) {
  return (
    <Section
      id="theming"
    ...
    >
      {modeResolutions ? (
        <div className="grid gap-6 lg:grid-cols-2">
          <SpecimenPanel label={`Engine decision — ${tenant.name} · light mode (boot)`}>
            <EngineDecision resolution={modeResolutions.light} />
          </SpecimenPanel>
          <SpecimenPanel label={`Engine decision — ${tenant.name} · dark mode`}>
            <EngineDecision resolution={modeResolutions.dark} />
          </SpecimenPanel>
        </div>
      ) : (
        <SpecimenPanel label={`Engine decision — ${tenant.name}`}>
          {resolution === null ? (
            ...
            <EngineDecision resolution={resolution} />
          )}
        </SpecimenPanel>
      )}
    </Section>
