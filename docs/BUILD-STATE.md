...
| 0.4 | Design system + theming (F2) | lead-ui-ux-designer | Design Review + **operator sign-off** (resolution 4b) | 🔨 Built; code-review found 1 BLOCKER (CSS injection via tenant font stacks) — remediation in flight; re-review then operator sign-off. NOT ready for freeze. |
...
**2026-07-07 · 0.4 F2 design system · Code Review (code quality/security, per CLAUDE.md rule 3) → REJECT (1 blocker, 4 minors)**
Design Review (self-owned) passed; 352 tests green; re-skin/token-parity/reduced-motion all verified genuine. But hostile-input probing against the theming engine found **[B1 BLOCKER] stored CSS injection via tenant font stacks**: colors are sanitized (malformed → Signal fallback) but font strings got only a non-empty check and reached the emitted `<style>` verbatim — a `}` in a font value breaks out of the CSS rule and injects app-wide CSS for every viewer of that tenant, on the real white-label production path. Confirmed with three live payloads. Fix dispatched to lead-ui-ux-designer (font-grammar validation in the brand-kit pipeline so both the string-sink and inline-style paths are covered) + regression tests + doc correction. Minors: named-CSS-color test gap, id-sanitization defense-in-depth, logo_url/custom_domain untrusted-field note for future consumers. **Gate held; F2 cannot reach operator sign-off until B1 is fixed and re-reviewed** — the freeze would otherwise bake the injection primitive into the foundation.
